...
class CasaDelLibroSupabaseUser extends BaseAuthUser {
  CasaDelLibroSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> casaDelLibroSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CasaDelLibroSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
